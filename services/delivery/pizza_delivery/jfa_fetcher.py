...
class JFAFetcher:
    ...
    async def sync_to_orm(self, session) -> int:
        ...
        n = 0
        for m in members:
            brand_display = m.brand_name or m.operator_name
            brand = upsert_brand(
                session,
                name=brand_display,
                source="jfa",
...
def _parse_member_index(html: str, *, source_url: str = "") -> list[JFAMember]:
    ...
    for tr in soup.find_all("tr"):
        ...
        if not name or name in seen:
            continue
        # ブランド名は col[1] の最初の行 (改行 / <br> で区切られる)
        brand = ""
        if len(tds) >= 2:
            # <br> より前の part だけ拾う
            first_text = tds[1].get_text("\n", strip=True).split("\n", 1)[0].strip()
            if first_text and not any(c in first_text for c in _CORP_MARKERS):
                brand = first_text[:100]
        industry = _pick_industry(td_texts)
        url = _pick_url(tr)
        members.append(
            JFAMember(
                operator_name=_canonicalize_corp_abbrev(name),
                brand_name=brand,
                industry=industry,
                url=url,
